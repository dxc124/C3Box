    ...
    def incremental_train(self, data_manager):
        ...
        if self._cur_task > 0:
            ...
            for p in self._network.oldfc.parameters():
                p.requires_grad = False
            for p in self._network.backbones[-1].parameters():
                p.requires_grad = False
            for p in self._network.backbones[-1].transformer.resblocks[11].parameters():
                p.requires_grad = True
        else:
            for p in self._network.backbones[0].parameters():
                p.requires_grad = False
            for p in self._network.backbones[0].transformer.resblocks[11].parameters():
                p.requires_grad = True

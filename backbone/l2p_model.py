...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, num_classes:int):
        ...
        self.prompt = Prompt(length=5, embed_dim=768,embedding_key='cls',prompt_init='uniform', prompt_pool=True,
                 prompt_key=True, pool_size=10, top_k=5, batchwise_prompt=True, prompt_key_init='uniform')
        self.use_prompt_mask = True
        self.new_positional_embedding = nn.Parameter(scale * torch.randn(1,(input_resolution // patch_size) ** 2 + 1+self.prompt.length * self.prompt.top_k, width))
    ...
    def forward_features(self, x, task_id=-1, cls_features=None, train=False):
        ...
        batched_prompt = res['batched_prompt']
        ...
        x = torch.cat([x[:,0:1,:],batched_prompt,x[:,1:,:]],dim=1)
        x = self.ln_pre(x)

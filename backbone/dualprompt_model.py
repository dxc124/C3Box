...
class Attention(nn.Module):
    ...
    def forward(self, x,  prompt=None):
        ...
        # B,N,3,n_h,h_dim->3,B,n_h,N,h_dim
...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None):
     ...
     #   self.attn = nn.MultiheadAttention(d_model, n_head)
        self.attn = Attention(dim=d_model, num_heads=n_head)
    ...
    def forward(self, x: torch.Tensor, prompt=None):
        x = x + self.attn(self.ln_1(x), prompt=prompt)
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, num_classes:int):
        super().__init__()
        self.input_resolution = input_resolution
        self.output_dim = output_dim
        ...
        self.g_prompt_length = 5
        g_prompt_shape = (num_g_prompt, 2, self.g_prompt_length, heads, width//heads)

        self.g_prompt = nn.Parameter(torch.randn(g_prompt_shape))
        nn.init.uniform_(self.g_prompt, -1, 1)

        self.e_prompt = EPrompt(length=5,embed_dim=width,embedding_key='cls',prompt_init='uniform',
                                prompt_pool=True,prompt_key=True,pool_size=10,top_k=1,batchwise_prompt=True,
                                prompt_key_init='uniform',num_layers=num_e_prompt,use_prefix_tune_for_e_prompt=True,
                                num_heads=heads,same_key_value=False)
    ...
    def forward_features(self, x, task_id=-1, cls_features=None, train=False):
        ...
        if self.use_prompt_mask and train:
            ...
            prompt_mask = single_prompt_mask.unsqueeze(0).expand(x.shape[0], -1)  # bs, topk
